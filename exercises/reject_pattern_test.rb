...
class RejectPatternTest < Minitest::Test
  ...
  def test_remove_numbers
    ...
    not_numbers = []
    elements.each do |el|
      not_numbers << unless el.is_a?(Integer)
    end
  ...
  def test_remove_floats
    ...
    not_numbers = []
    elements.each do |el|
      not_numbers << unless el.is_a?(Float)
      end
  ...
  def test_remove_animals_starting_with_vowels
    ...
    remaining = []
    animals.each do |a|
      remaining << unless a.start_with?(/[aeiouy]/)
      end
  ...
  def test_remove_capitalized_words
    ...
    remaining = []
    words.each do |word|
      remaining << unless word[0..word.length] =~ /[A..Z]/
      end
  ...
  def test_remove_arrays
    ...
    remaining = []
    elements.each do |el|
      remaining << unless el.is_a?(Array)
      end
